-- BET+AI — WERSJA 1777
-- Naprawa starego odblokowania 1/1 dla osiągnięcia „Krytyk Bukmacherski”.
-- Nowy cel to 50 ocen i odznaka ma być zablokowana, dopóki current_value < 50.

begin;

-- 1. Reset istniejących błędnie odblokowanych rekordów.
update public.tipster_achievement_progress
set
  target_value = 50,
  unlocked = (current_value >= 50),
  unlocked_at = case
...
-- 2. Poprawka upsertu:
--    pozostałe odznaki zachowują trwałe odblokowanie,
--    ale Krytyk Bukmacherski po zmianie celu działa według aktualnego progu 50.
...
begin
  ...
  do update set
    ...
    unlocked = case
      when excluded.achievement_key = 'krytyk-bukmacherski'
        then excluded.current_value >= excluded.target_value
      else
        public.tipster_achievement_progress.unlocked or excluded.unlocked
    end,

    unlocked_at = case
      when excluded.achievement_key = 'krytyk-bukmacherski'
           and excluded.current_value < excluded.target_value
        then null
      when excluded.achievement_key = 'krytyk-bukmacherski'
           and excluded.current_value >= excluded.target_value
        then coalesce(public.tipster_achievement_progress.unlocked_at, now())
      else
        coalesce(
...
-- 3. Przeliczenie wszystkich profili.
do $$
declare
  profile_row record;
begin
