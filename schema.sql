...
function AdminPayoutsView({ user, requests = [], onUpdateStatus, onRunCron }) {
  ...
  const normalizedRequests = useMemo(() => requests.map(request => ({
    ...request,
  ...
  const processingRequests = normalizedRequests.filter(request => request.status === 'processing')
  const paidRequests = normalizedRequests.filter(request => request.status === 'paid')
  const failedRequests = normalizedRequests.filter(request => request.status === 'failed' || request.stripe_status === 'failed')
  const rejectedRequests = normalizedRequests.filter(request => request.status === 'rejected')
  ...
  const totalAll = normalizedRequests.reduce((sum, request) => sum + request.amountNumber, 0)
  ...
  const filteredRequests = useMemo(() => {
    ...
    return normalizedRequests.filter(request => {
      const matchesQuery = !q || [request.user_id, request.id, request.status, request.stripe_transfer_id, request.stripe_status, String(request.amountNumber)].some(value => String(value || '').toLowerCase().includes(q))
      const matchesStatus = statusFilter === 'all' || request.status === statusFilter
      const matchesAmount = amountFilter === 'all' || (amountFilter === 'ready' && request.amountNumber >= 50 && request.status === 'pending') || (amountFilter === 'small' && request.amountNumber < 50) || (amountFilter === 'large' && request.amountNumber >= 100)
      const matchesDate = dateFilter === 'all' || (request.createdDate && dateFilter === 'today' && request.createdDate.toDateString() === now.toDateString()) || (request.createdDate && dateFilter === '7d' && (now - request.createdDate) <= 7 * 24 * 60 * 60 * 1000) || (request.createdDate && dateFilter === '30d' && (now - request.createdDate) <= 30 * 24 * 60 * 60 * 1000)
      return matchesQuery && matchesStatus && matchesAmount && matchesDate
    }).sort((a, b) => {
      const order = { processing: 5, pending: 4, failed: 3, rejected: 2, paid: 1 }
      return (order[b.status] || 0) - (order[a.status] || 0) || (b.createdDate?.getTime?.() || 0) - (a.createdDate?.getTime?.() || 0)
    })
  }, [normalizedRequests, query, statusFilter, amountFilter, dateFilter])

  const selectedRequests = filteredRequests.filter(request => selectedIds.includes(request.id))
  const selectableRequests = filteredRequests.filter(request => request.status === 'pending')
  const selectedPending = selectedRequests.filter(request => request.status === 'pending')
  ...
  const getStatusLabel = (status) => ({ pending: 'Oczekująca', processing: 'W trakcie', paid: 'Wypłacona', rejected: 'Odrzucona', failed: 'Błąd' }[status] || status || 'pending')
  const toggleSelected = (id) => setSelectedIds(prev => prev.includes(id) ? prev.filter(item => item !== id) : [...prev, id])
  const toggleAllVisible = () => {
    const ids = selectableRequests.map(request => request.id)
  ...
  const bulkUpdate = async (status) => {
    for (const request of selectedPending) {
      if (status !== 'rejected' && request.amountNumber < 50) continue
      await onUpdateStatus(request.id, status)
    }
    setSelectedIds([])
  }
  const exportCsv = () => {
    const header = ['id','user_id','created_at','amount','status','stripe_status','stripe_transfer_id']
    const rows = filteredRequests.map(request => header.map(key => `"${String(request[key] ?? '').replaceAll('"','""')}"`).join(','))
    const blob = new Blob([[header.join(','), ...rows].join('\n')], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `admin-wyplaty-${new Date().toISOString().slice(0,10)}.csv`
    a.click()
    URL.revokeObjectURL(url)
  }

  if (!profile.isAdmin) {
    return <section className="admin-payout-page admin-payout-ux-pro"><div className="admin-denied"><strong>Brak dostępu</strong><span>Ten panel jest dostępny tylko dla administratora.</span></div></section>
  }

  return (
    <section className="admin-payout-page admin-payout-ux-pro">
      <div className="admin-payout-hero admin-payout-hero-ux">
        <div>
          <div className="admin-kicker">Stripe Connect Control Center</div>
          <h1>Admin wypłaty</h1>
          <p>Ultra PRO panel do realnych wypłat: approve, reject, bulk actions, transfer ID, CSV i gotowość pod cron.</p>
        </div>
        <div className="admin-payout-badge">ADMIN PRO</div>
      </div>

      <div className="admin-payout-stats admin-payout-stats-pro admin-payout-stats-ux">
        <div><span>Zgłoszenia</span><b>{normalizedRequests.length}</b><em>Wszystkie requesty</em></div>
        <div><span>Pending</span><b>{pendingRequests.length}</b><em>{totalPending.toFixed(2)} zł oczekuje</em></div>
        <div><span>Wypłacone</span><b>{paidRequests.length}</b><em>{totalPaid.toFixed(2)} zł zrealizowane</em></div>
        <div><span>Odrzucone</span><b>{rejectedRequests.length}</b><em>{failedRequests.length} wymaga uwagi</em></div>
      </div>

      <div className="admin-cron-card admin-cron-card-ux">
        <div>
          <strong>Automatyczne wypłaty — cron ready <small>Aktywne</small></strong>
          <span>Endpoint <code>/.netlify/functions/process-payouts</code> obsługuje pending wypłaty od 50 zł, blokuje duplikaty statusem processing i używa idempotency key.</span>
        </div>
        <div className="admin-cron-actions"><span>Do automatu: <b>{payableRequests.length}</b> • Suma: <b>{totalPending.toFixed(2)} zł</b></span><button type="button" className="cron-run-button" onClick={onRunCron} disabled={!automationReady}>{automationReady ? 'Uruchom teraz' : 'Brak pending ≥ 50 zł'}</button></div>
      </div>

      <div className="admin-payout-toolbar">
        <div className="admin-search-field"><span>⌕</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Szukaj po ID użytkownika, kwocie, statusie, Stripe ID..." /></div>
        <select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">Status: wszystkie</option><option value="pending">Pending</option><option value="processing">Processing</option><option value="paid">Paid</option><option value="rejected">Rejected</option><option value="failed">Failed</option></select>
        <select value={amountFilter} onChange={event => setAmountFilter(event.target.value)}><option value="all">Kwota: wszystkie</option><option value="ready">Gotowe ≥ 50 zł</option><option value="small">Poniżej 50 zł</option><option value="large">Duże ≥ 100 zł</option></select>
        <select value={dateFilter} onChange={event => setDateFilter(event.target.value)}><option value="all">Data: wszystkie</option><option value="today">Dzisiaj</option><option value="7d">Ostatnie 7 dni</option><option value="30d">Ostatnie 30 dni</option></select>
        <button type="button" className="admin-filter-button" onClick={() => { setQuery(''); setStatusFilter('all'); setAmountFilter('all'); setDateFilter('all') }}>Reset</button>
      </div>

      <div className="admin-bulk-bar">
        <div><strong>Zaznaczone: {selectedPending.length}</strong><span>Widoczne: {filteredRequests.length} • Gotowe do wypłaty: {payableRequests.length} • Całość: {totalAll.toFixed(2)} zł</span></div>
        <div className="admin-bulk-actions"><button type="button" onClick={toggleAllVisible}>{selectableRequests.every(r => selectedIds.includes(r.id)) && selectableRequests.length ? 'Odznacz' : 'Zaznacz pending'}</button><button type="button" disabled={!selectedPending.length} onClick={() => bulkUpdate('paid')}>Zatwierdź wypłaty ({selectedPending.length})</button><button type="button" className="danger" disabled={!selectedPending.length} onClick={() => bulkUpdate('rejected')}>Odrzuć</button><button type="button" onClick={exportCsv}>Eksport CSV</button></div>
      </div>

      <div className="admin-payout-table admin-payout-table-ux">
        <div className="admin-payout-row header"><span></span><span>User</span><span>Data</span><span>Kwota</span><span>Status</span><span>Stripe Transfer ID</span><span>Akcje</span></div>
        {filteredRequests.length ? filteredRequests.map(request => {
          const canPay = request.status === 'pending' && request.amountNumber >= 50
          return <div className={`admin-payout-row status-${request.status}`} key={request.id}><span className="admin-check-cell"><button type="button" className={`admin-check ${selectedIds.includes(request.id) ? 'selected' : ''}`} disabled={request.status !== 'pending'} onClick={() => toggleSelected(request.id)}>{selectedIds.includes(request.id) ? '✓' : ''}</button></span><span className="admin-user-cell"><b>{request.user_email || request.username || (request.user_id ? request.user_id.slice(0, 8) : 'Użytkownik')}</b><small>{request.user_id ? request.user_id.slice(0, 10) + '...' : request.id?.slice?.(0, 10) || '—'}</small></span><span>{request.created_at ? new Date(request.created_at).toLocaleString('pl-PL') : '—'}</span><span><b>{request.amountNumber.toFixed(2)} zł</b></span><span className={`payout-status ${request.status || 'pending'}`}>{getStatusLabel(request.status)}</span><span className="admin-stripe-cell"><b>{request.stripe_status || (request.status === 'rejected' ? 'rejected' : '—')}</b><small>{getStripeLabel(request)}</small></span><span className="admin-actions">{request.status === 'pending' ? <><button type="button" disabled={!canPay} onClick={() => onUpdateStatus(request.id, 'paid')}>Zatwierdź</button><button type="button" className="danger" onClick={() => onUpdateStatus(request.id, 'rejected')}>Odrzuć</button></> : <button type="button" className="admin-secondary-action">Szczegóły</button>}</span></div>
        }) : <div className="admin-empty"><strong>Brak zgłoszeń dla filtrów</strong><span>Zmień filtr albo wyczyść wyszukiwanie.</span></div>}
      </div>

      <div className="stripe-connect-note stripe-connect-note-ux"><strong>System finalizacji wypłat</strong><span>Manual approve wykonuje realny Stripe transfer. Cron może przetwarzać pending automatycznie, jeżeli ustawisz CRON_SECRET i harmonogram Netlify.</span><button type="button">Dokumentacja</button></div>
    </section>
  )
}
