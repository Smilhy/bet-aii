...
const { assertWorkerToken } = require('./_lib/queue-ai-background')
...
const VERSION = '1867.4-ograc-buka-background-active-v1'
...
async function fetchOddsGroups(dates, errors) {
  const map = new Map()
  const maxPages = clamp(process.env.OGRAC_BUKA_ODDS_MAX_PAGES || 2, 1, 20)
...
function pendingStillBlocks(row, graceHours = 10) {
  const eventRaw = row?.event_time || row?.kickoff_time || row?.match_time || row?.commence_time
  const eventMs = Date.parse(eventRaw || '')
  if (Number.isFinite(eventMs)) return Date.now() <= eventMs + graceHours * 60 * 60 * 1000
  const createdMs = Date.parse(row?.created_at || '')
  return !Number.isFinite(createdMs) || Date.now() - createdMs < 36 * 60 * 60 * 1000
}
...
async function readProgressionState(supabase, targetProfit) {
  ...
  const pending = rows.filter(row => normalizeStatus(row.status || row.result_status || row.result || row.settlement_status) === 'pending' && pendingStillBlocks(row))
...
function buildApiEvidence(candidate, predictionRow, injuryMap, settings) {
  ...
  if (side) {
    ...
    const impliedPct = (1 / Math.max(1.01, n(candidate?.pick?.odds, 1))) * 100
    const requiredApiProbability = Math.max(settings.minApiProbability, Math.min(55, impliedPct - 2))
    supported = winnerOk && apiProbability >= requiredApiProbability
  ...
  return {
    ...
    requiredApiProbability: side ? round(Math.max(settings.minApiProbability, Math.min(55, (1 / Math.max(1.01, n(candidate?.pick?.odds, 1))) * 100 - 2)), 1) : 0,
...
// WERSJA 1867.4: długi skan działa w tle. Harmonogram uruchamia lekki trigger.


async function recordRun(supabase, status, picksCreated, message, errors = []) {
  try {
    const detail = [message, ...(Array.isArray(errors) ? errors : [])].filter(Boolean).join(' | ').slice(0, 1000)
    await supabase.from('ai_pick_runs').insert({
      source: VERSION,
      picks_created: Number(picksCreated || 0),
...
exports.handler = async function(event) {
  ...
  if (!assertWorkerToken(event, 'publish-ograc-buka-background')) return json(401, { ok: false, error: 'Unauthorized background worker invocation' })
  ...
  const minMinutes = clamp(q.min_minutes_before_start || process.env.OGRAC_BUKA_MIN_MINUTES_BEFORE_START || 60, 30, 720)
  const maxHours = clamp(q.max_hours_ahead || process.env.OGRAC_BUKA_MAX_HOURS_AHEAD || 72, 6, 96)
  ...
  const settings = {
    ...
    minProbability: clamp(q.min_probability || process.env.OGRAC_BUKA_MIN_PROBABILITY || 0.22, 0.15, 0.85),
    ...
    maxStake: 1,
    targetProfit: 0,
    apiCandidates: clamp(q.api_candidates || process.env.OGRAC_BUKA_API_CANDIDATES || 5, 1, 8),
    minApiScore: clamp(q.min_api_score || process.env.OGRAC_BUKA_MIN_API_SCORE || 48, 30, 95),
    minApiProbability: clamp(q.min_api_probability || process.env.OGRAC_BUKA_MIN_API_PROBABILITY || 30, 20, 85),
    minCombinedScore: clamp(q.min_combined_score || process.env.OGRAC_BUKA_MIN_COMBINED_SCORE || 52, 30, 95),
    ...
    minComparison: clamp(q.min_comparison || process.env.OGRAC_BUKA_MIN_COMPARISON || 40, 30, 80),
    minConservativeEdge: clamp(q.min_conservative_edge || process.env.OGRAC_BUKA_MIN_CONSERVATIVE_EDGE || 0.005, 0, 0.15),
    cooldownHours: clamp(q.cooldown_hours || process.env.OGRAC_BUKA_COOLDOWN_HOURS || 6, 4, 72)
  }
  ...
  if (!dryRun && progressionState.pending.length > 0) {
    await recordRun(supabase, 'success', 0, `previous_pick_pending=${progressionState.pending.length}`, errors)
    return json(200, {
      ok: true,
      version: VERSION,
      author: AUTHOR_NAME,
  ...
  if (!dryRun && !force) {
    try {
      ...
      if (latest && hoursSince(latest.created_at) < settings.cooldownHours) {
        await recordRun(supabase, 'success', 0, `cooldown ${settings.cooldownHours}h`, errors)
        return json(200, {
          ok: true,
          version: VERSION,
          author: AUTHOR_NAME,
  ...
  if (!selected) {
    await recordRun(supabase, 'success', 0, `no_candidate fixtures=${fixtures.length} odds=${oddsGroups.size} market=${uniqueFixtures.length} api=${apiCandidates.length}`, errors)
    return json(200, {
      ok: true,
      version: VERSION,
      author: AUTHOR_NAME,
  ...
  try {
    ...
    await recordRun(supabase, 'error', 0, error.message || String(error), errors)
    return json(500, {
      ok: false,
      version: VERSION,
      author: AUTHOR_NAME,
