...
const VERSION = '1867.2-typer-expert-balanced-v2'
...
exports.handler = async function(event) {
  ...
  const minMinutes = clamp(q.min_minutes_before_start || process.env.TYPER_EXPERT_MIN_MINUTES_BEFORE_START || 75, 45, 720)
  const maxHours = clamp(q.max_hours_ahead || process.env.TYPER_EXPERT_MAX_HOURS_AHEAD || 48, 6, 72)
  ...
  const settings = {
    ...
    apiCandidates: clamp(q.api_candidates || process.env.TYPER_EXPERT_API_CANDIDATES || 7, 1, 8),
    minApiScore: clamp(q.min_api_score || process.env.TYPER_EXPERT_MIN_API_SCORE || 52, 40, 95),
    minApiProbability: clamp(q.min_api_probability || process.env.TYPER_EXPERT_MIN_API_PROBABILITY || 50, 40, 85),
    minCombinedScore: clamp(q.min_combined_score || process.env.TYPER_EXPERT_MIN_COMBINED_SCORE || 55, 40, 95),
