...
const VERSION = '1867.4-typer-expert-background-active-v1'
...
async function fetchOddsGroups(dates, errors) {
  const map = new Map()
  const maxPages = clamp(process.env.TYPER_EXPERT_ODDS_MAX_PAGES || 2, 1, 20)
...
exports.handler = async function(event) {
  ...
  if (!assertWorkerToken(event, 'publish-typer-expert-background')) return json(401, { ok: false, error: 'Unauthorized background worker invocation' })
  ...
  const minMinutes = clamp(q.min_minutes_before_start || process.env.TYPER_EXPERT_MIN_MINUTES_BEFORE_START || 60, 30, 720)
  const maxHours = clamp(q.max_hours_ahead || process.env.TYPER_EXPERT_MAX_HOURS_AHEAD || 72, 6, 96)
  ...
  const settings = {
    ...
    minProbability: clamp(q.min_probability || process.env.TYPER_EXPERT_MIN_PROBABILITY || 0.20, 0.15, 0.85),
    ...
    maxOddsOutlierRatio: clamp(q.max_odds_outlier_ratio || process.env.TYPER_EXPERT_MAX_ODDS_OUTLIER_RATIO || 1.12, 1.01, 1.2),
    ...
    minApiScore: clamp(q.min_api_score || process.env.TYPER_EXPERT_MIN_API_SCORE || 44, 30, 95),
    minApiProbability: clamp(q.min_api_probability || process.env.TYPER_EXPERT_MIN_API_PROBABILITY || 26, 18, 85),
    minCombinedScore: clamp(q.min_combined_score || process.env.TYPER_EXPERT_MIN_COMBINED_SCORE || 48, 30, 95),
    apiWeight: clamp(q.api_weight || process.env.TYPER_EXPERT_API_WEIGHT || 0.55, 0.2, 0.8),
    marketWeight: 0,
    requireApiPrediction: boolEnv(q.require_api_prediction ?? process.env.TYPER_EXPERT_REQUIRE_API_PREDICTION, false),
