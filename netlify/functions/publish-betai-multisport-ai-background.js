...
const VERSION = '1867.4-betai-background-active-v1'
...
async function fetchOddsGroups(dates, errors) {
  const map = new Map()
  const maxPages = clamp(process.env.BETAI_VALUE_V2_ODDS_MAX_PAGES || 2, 1, 20)
...
// WERSJA 1867.4: długi skan działa jako Netlify Background Function. Harmonogram uruchamia lekki trigger.
...
exports.handler = async function(event) {
  ...
  if (!assertWorkerToken(event, 'publish-betai-multisport-ai-background')) return json(401, { ok: false, error: 'Unauthorized background worker invocation' })
  ...
  const minMinutes = clamp(q.min_minutes_before_start || process.env.BETAI_VALUE_V2_MIN_MINUTES_BEFORE_START || 60, 30, 720)
  const maxHours = clamp(q.max_hours_ahead || process.env.BETAI_VALUE_V2_MAX_HOURS_AHEAD || 72, 6, 96)
  const cooldownHours = clamp(q.cooldown_hours || process.env.BETAI_VALUE_V2_COOLDOWN_HOURS || 6, 4, 48)

  const settings = {
    minBooks: clamp(q.min_books || process.env.BETAI_VALUE_V2_MIN_BOOKS || 2, 2, 15),
    ...
    minProbability: clamp(q.min_probability || process.env.BETAI_VALUE_V2_MIN_PROBABILITY || 0.18, 0.15, 0.8),
    minEdge: clamp(q.min_edge || process.env.BETAI_VALUE_V2_MIN_EDGE || 0.005, 0, 0.2),
    maxProbabilitySpread: clamp(q.max_probability_spread || process.env.BETAI_VALUE_V2_MAX_PROBABILITY_SPREAD || 0.10, 0.01, 0.18),
    maxOddsOutlierRatio: clamp(q.max_odds_outlier_ratio || process.env.BETAI_VALUE_V2_MAX_ODDS_OUTLIER_RATIO || 1.15, 1.01, 1.3)
  }
  ...
  if (!dryRun && !force) {
    try {
      ...
      if (recentPick) {
        ...
        await recordRun(supabase, 'success', 0, `cooldown ${cooldownHours}h`, errors)
        return json(200, {
          ok: true,
          version: VERSION,
          author: AUTHOR_NAME,
  ...
  if (!selected) {
    await recordRun(supabase, 'success', 0, `no_candidate fixtures=${fixtures.length} odds=${oddsGroups.size} candidates=${uniqueFixtures.length}`, errors)
    return json(200, {
      ok: true,
      version: VERSION,
      author: AUTHOR_NAME,
