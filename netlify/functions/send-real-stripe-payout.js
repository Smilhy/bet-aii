...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!request_id) return { statusCode: 400, body: JSON.stringify({ error: 'Missing request_id' }) };
    ...
    if (!amountGrosze || amountGrosze < 100) throw new Error('INVALID_PAYOUT_AMOUNT');
    ...
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ error: error.message || 'Stripe payout error' })
    };
  }
};
