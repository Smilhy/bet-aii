-- WERSJA 978 — blokada jednej logiki statystyk
-- Uruchom w Supabase SQL Editor.
-- Stała zasada:
-- 1. Typy = wszystkie typy.
-- 2. Profit/Bilans = tylko typy rozliczone.
-- 3. Stawka do Yield/ROI = tylko stawka typów rozliczonych.
-- 4. Pending nie wchodzi do stawki ROI/Yield.
...
begin
  ...
  where p.id = p_profile_id
    and not exists (
      select 1 from public.tips t where t.author_id = p_profile_id
    );
end;
$func$;
