-- supabase/version_1053_typy_ai_fetch_hero_fix.sql
-- Bezpieczne dopięcie pod Typy AI 1053.
...
drop policy if exists "ai_leagues_catalog_select_all_v1053" on public.ai_leagues_catalog;
create policy "ai_leagues_catalog_select_all_v1053"
on public.ai_leagues_catalog
for select
to authenticated
using (true);

drop policy if exists "ai_leagues_catalog_service_write_v1053" on public.ai_leagues_catalog;
create policy "ai_leagues_catalog_service_write_v1053"
on public.ai_leagues_catalog
for all
to service_role
using (true)
...
create index if not exists tips_ai_external_key_idx_v1053 on public.tips(ai_external_key);
create index if not exists tips_ai_stats_idx_v1053 on public.tips(sport, league, market, status);

select 'v1053 typy ai fetch hero fix ready' as status;
