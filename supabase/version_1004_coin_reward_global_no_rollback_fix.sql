...
-- supabase/version_1004_coin_reward_global_no_rollback_fix.sql
-- Mocniejsza naprawa cofania Coin: RPC zawsze zwraca saldo po update i zapisuje powiadomienie z reward_tokens.
...
begin
  ...
  on conflict (email)
  do update set
    balance = greatest(coalesce(public.betai_token_wallets.balance, 0) + 1, coalesce(public.betai_token_wallets.balance, 0)),
    user_id = coalesce(public.betai_token_wallets.user_id, excluded.user_id),
    welcome_bonus_claimed = coalesce(public.betai_token_wallets.welcome_bonus_claimed, true),
    updated_at = now()
...
select 'v1004 coin reward global no rollback fixed' as status;
