...
-- supabase/version_1011_weekly_reward_claims_monday_1am_lock.sql
-- Nagrody rankingowe:
-- - Odebrane są trzymane w bazie, więc deploy nowej paczki nie resetuje przycisku.
-- - Reset tygodnia: poniedziałek 01:00.
-- - Nagroda nadal = 1 żeton.
-- - Saldo zostaje w betai_token_wallets i historia w betai_token_transactions.
...
alter table if exists public.betai_ranking_challenge_claims
...
-- Unikalność na tydzień i wyzwanie.
do $$
begin
  if not exists (
    select 1
...
select 'v1011 weekly reward claims locked, reset monday 01:00' as status;
