-- supabase/version_1052_typy_ai_quality_fix.sql
-- Bezpieczne dopięcie pod Typy AI 1052: kolumny, indeksy i katalog lig.
-- Nie rusza finansów, wypłat, Stripe ani auto-settle.
...
begin
  if not exists (
    ...
    alter table public.ai_leagues_catalog add constraint ai_leagues_catalog_sport_league_key unique (sport, league);
  end if;
...
drop policy if exists "ai_leagues_catalog_select_all_v1052" on public.ai_leagues_catalog;
create policy "ai_leagues_catalog_select_all_v1052"
on public.ai_leagues_catalog
for select
to authenticated
using (true);

drop policy if exists "ai_leagues_catalog_service_write_v1052" on public.ai_leagues_catalog;
create policy "ai_leagues_catalog_service_write_v1052"
on public.ai_leagues_catalog
for all
to service_role
using (true)
...
create index if not exists tips_ai_external_key_idx_v1052 on public.tips(ai_external_key);
create index if not exists tips_ai_stats_idx_v1052 on public.tips(sport, league, market, status);

select 'v1052 typy ai quality fix ready' as status;
