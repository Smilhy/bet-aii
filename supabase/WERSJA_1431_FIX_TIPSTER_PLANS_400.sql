-- WERSJA 1431 — FIX tipster_plans 400 / TOP TYPERZY
-- Wklej w Supabase SQL Editor i uruchom.
--
-- Cel:
-- - czerwone 400 na tipster_plans?select=plan_key...
-- - stworzyć / uzupełnić tabelę tipster_plans pod zapisy planów typera,
-- - dodać unikalny indeks pod upsert: onConflict tipster_id,plan_key,
-- - odświeżyć schema cache PostgREST.
--
-- Nie usuwa danych.
...
-- Jeżeli stara tabela miała tipster_id jako uuid, a frontend wysyła też email/nick,
-- tekstowy typ jest najbezpieczniejszy dla tej funkcji.
do $$
begin
  if exists (
    select 1
...
select 'WERSJA 1431 tipster_plans 400 fixed' as status;
