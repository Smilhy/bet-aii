-- WERSJA 1571 — FIX: SPOŁECZNOŚĆ / NAGRODY / ODBIERZ +1 COIN
-- Wklej CAŁOŚĆ w Supabase SQL Editor i uruchom RAZ.
-- Naprawia przyciski „Odbierz +1 coin”, gdy pasek pokazuje 1/1, ale kliknięcie nic nie robi.
-- Przyczyna: poprzednia funkcja claim_community_reward_v1437 obsługiwała tylko daily_chat/daily_post/daily_active,
-- a zakładka „Nagrody społeczności” woła też: first_post, first_comment, first_chat, social_value.
...
-- Kolumny aktywności. Jeśli tabele już istnieją, tylko dopisuje brakujące kolumny.
...
-- Nadpisujemy tę samą nazwę funkcji, której używa frontend.
...
begin
  ...
  if v_recent_claim then
    return jsonb_build_object('claimed', false, 'already_claimed', true, 'message', 'Ta nagroda była już odebrana w ostatnich 24h.', 'rule', 'one_claim_per_24h');
  end if;

  -- Nagrody z zakładki „Nagrody społeczności”
  if v_key = 'first_post' then
    select count(*)::integer into v_count
    from public.community_posts p
    where p.author_id = p_user_id or lower(coalesce(p.author_email, '')) = v_email;
    v_done := coalesce(v_count, 0) >= 1;
    ...
    select coalesce(sum(coalesce(p.likes_count, 0)), 0)::integer into v_likes
    from public.community_posts p
    where p.author_id = p_user_id or lower(coalesce(p.author_email, '')) = v_email;
    v_count := v_likes;
    v_done := coalesce(v_likes, 0) >= 3;

  -- Misje dzienne z lewego panelu
  elsif v_key = 'daily_chat' then
    select count(*)::integer into v_count
    from public.community_chat_messages m
    ...
    select exists (
      ...
      where r.reward_key = 'daily_chat' and r.created_at >= v_cutoff
        and (r.user_id = p_user_id or lower(coalesce(r.email, '')) = v_email)
    ) into v_daily_chat_claimed;

    select exists (
      select 1 from public.community_reward_claims r
      where r.reward_key = 'daily_post' and r.created_at >= v_cutoff
        and (r.user_id = p_user_id or lower(coalesce(r.email, '')) = v_email)
    ...
    return jsonb_build_object('claimed', false, 'message', 'Nieznana nagroda: ' || v_key, 'rule', 'unknown_reward');
  end if;
  ...
  if not coalesce(v_done, false) then
    return jsonb_build_object(
      ...
      'message', 'Najpierw wykonaj wymaganą akcję.',
  ...
  return jsonb_build_object(
    ...
    'message', 'Nagroda odebrana. Dodano +1 coin.'
  );
end;
$$;
...
select 'OK: WERSJA 1571 community rewards claim fix installed' as status;
